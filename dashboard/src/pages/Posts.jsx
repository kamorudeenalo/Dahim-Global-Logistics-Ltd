...
export default function Posts() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const statusRequests = ['publish', 'draft', 'future'].map(status =>
        listItemsPaged('posts', { status, per_page: 1 }).catch(() => ({ total: 0 }))
      );
      ...
      setCounts({
        all: published.total + drafts.total + scheduled.total,
        publish: published.total,
        draft: drafts.total,
        future: scheduled.total,
        trash: trash.total,
      });
    } catch (err) {
    ...
    try {
      ...
      setEditing({
        ...
        form: {
          ...
          slug: full.slug || '',
          date: toSiteDateTime(new Date(full.date), siteTimezone),
          categories: Array.isArray(full.categories) ? full.categories : [],
          tags: Array.isArray(full.tags) ? full.tags : [],
          featured_media: Number(full.featured_media || 0),
          author: Number(full.author || 0) || '',
          focus_keyword: full.meta?.dahim_focus_keyword || '',
          seo_title: full.meta?.dahim_seo_title || '',
          meta_description: full.meta?.dahim_meta_description || '',
        },
      });
      setNewCategory('');
      setNewTag('');
    } catch (err) {
      push(err instanceof ApiError ? err.message : 'Could not open this Insight.', 'error');
    }
  }
  ...
  function setField(key, value) {
    setEditing(prev => ({ ...prev, dirty: true, form: { ...prev.form, [key]: value } }));
  }
  ...
  function setTitle(value) {
    setEditing(prev => ({
      ...prev,
      dirty: true,
      form: { ...prev.form, title: value, slug: prev.slugManual ? prev.form.slug : slugify(value) },
    }));
  }

  function setSlug(value) {
    setEditing(prev => ({ ...prev, dirty: true, slugManual: true, form: { ...prev.form, slug: slugify(value) } }));
  }
  ...
  const savePost = async (intent = 'draft') => {
    ...
    if (intent === 'schedule' && !isFutureDate(form.date, siteTimezone)) {
      push('Choose a future publication date before scheduling.', 'error'); return;
    }
    setSaving(true);
    try {
      ...
      const payload = {
        ...
        status, slug: form.slug || undefined, categories: form.categories, tags: form.tags,
        featured_media: Number(form.featured_media || 0),
        meta: {
          dahim_focus_keyword: form.focus_keyword.trim(),
          dahim_seo_title: form.seo_title.trim(),
          dahim_meta_description: form.meta_description.trim(),
        },
      };
      if (form.author) payload.author = Number(form.author);
      if (form.date) payload.date = localSiteDateTimeToUtc(form.date, siteTimezone);
      if (editing.id) await updateItem('posts', editing.id, payload);
      else await createItem('posts', payload);
      setEditing(null);
      setSelected(new Set());
      await load();
      ...
      push(err instanceof ApiError ? err.message : 'Could not save this Insight.', 'error');
  ...
  async function handleFeaturedUpload(e) {
    ...
    try {
      ...
      setField('featured_media', Number(media.id));
      push('Featured image uploaded.', 'success');
  ...
  async function addCategory() {
    ...
    try {
      ...
      setField('categories', [...editing.form.categories, Number(cat.id)]);
      setNewCategory(''); push('Category added.', 'success');
  ...
  async function addTag() {
    ...
    try {
      ...
      setField('tags', [...editing.form.tags, Number(tag.id)]);
      setNewTag(''); push('Tag added.', 'success');
  ...
  async function bulk(action, idsOverride = null) {
    ...
    for (const id of ids) {
      try {
        if (action === 'delete') await deleteItem('posts', id);
  ...
  if (editing) return (
    <InsightEditor
      editing={editing} categories={categories} tags={tags} authors={authors}
      saving={saving} uploading={uploading} siteTimezone={siteTimezone}
      newCategory={newCategory} setNewCategory={setNewCategory} newTag={newTag} setNewTag={setNewTag}
      onClose={closeEditor} onSave={savePost} onFeaturedUpload={handleFeaturedUpload}
      setField={setField} setTitle={setTitle} setSlug={setSlug} addCategory={addCategory} addTag={addTag} push={push}
    />
  );
  ...
  return (
    <div className="insights-page">
      <div className="insights-head">
        <div><div className="eyebrow">CONTENT</div><h1>Insights</h1><p>Write, optimize and publish articles for the Dahim website.</p></div>
        <button className="btn btn-primary insights-new-btn" onClick={openNew}><IconPlus /> New Insight</button>
      </div>
      <div className="insights-tabs">
        {TABS.map(([key, label]) => <button key={key} className={tab === key ? 'active' : ''} onClick={() => setTab(key)}>{label}<span>{counts[key] || 0}</span></button>)}
      </div>
      <div className="insights-toolbar">
        <div className="insights-search"><span>⌕</span><input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search Insights" /></div>
        ...
        <div className="insights-bulk"><select value={bulkAction} onChange={e => setBulkAction(e.target.value)}><option value="">Bulk actions</option>{tab === 'trash' ? <><option value="restore">Restore</option><option value="delete">Delete permanently</option></> : <><option value="trash">Move to Bin</option><option value="publish">Publish</option><option value="draft">Move to Drafts</option></>}</select><button className="btn btn-outline btn-sm" onClick={() => { if (bulkAction) { bulk(bulkAction); setBulkAction(''); } }}>Apply</button></div>
      </div>
      {loading ? <div className="center-loading"><div className="spinner" /></div> : !items.length ? (
        <div className="insights-empty"><div className="insights-empty-icon">✦</div><h3>{tab === 'trash' ? 'Your Bin is empty' : 'No Insights found'}</h3><p>Start publishing useful information for Dahim customers and partners.</p>{tab !== 'trash' && <button className="btn btn-primary" onClick={openNew}><IconPlus /> Create your first Insight</button>}</div>
      ) : <>
        <div className="insights-table-card"><table className="insights-table"><thead><tr><th className="check-col"><input type="checkbox" checked={allVisibleSelected} onChange={e => setSelected(prev => { const next = new Set(prev); items.forEach(item => e.target.checked ? next.add(item.id) : next.delete(item.id)); return next; })} /></th><th>Insight</th><th>Author</th><th>Category</th><th>SEO</th><th>Status</th><th>Date</th><th /></tr></thead><tbody>
          {items.map(item => <InsightRow key={item.id} item={item} categories={categories} authors={authors} selected={selected.has(item.id)} tab={tab} onSelect={checked => setSelected(prev => { const next = new Set(prev); checked ? next.add(item.id) : next.delete(item.id); return next; })} onEdit={() => openEdit(item)} onTrash={() => bulk(tab === 'trash' ? 'delete' : 'trash', [item.id])} onRestore={() => bulk('restore', [item.id])} />)}
        </tbody></table></div>
        <div className="insights-pagination"><span>Page {page} of {totalPages}</span><div><button className="btn btn-outline btn-sm" disabled={page <= 1} onClick={() => setPage(p => p - 1)}>Previous</button><button className="btn btn-outline btn-sm" disabled={page >= totalPages} onClick={() => setPage(p => p + 1)}>Next</button></div></div>
      </>}
    </div>
  );
}
...
function InsightEditor({ editing, categories, tags, authors, saving, uploading, siteTimezone, newCategory, setNewCategory, newTag, setNewTag, onClose, onSave, onFeaturedUpload, setField, setTitle, setSlug, addCategory, addTag, push }) {
  const { form } = editing;
  ...
  const previewUrl = `${publicBase}/insights/${form.slug || 'your-insight-slug'}`;
  ...
  return <div className="insight-editor-page">
    <div className="insight-editor-header"><div><button className="back-link" onClick={onClose}>← All Insights</button><div className="eyebrow">INSIGHT EDITOR</div><h1>{editing.id ? 'Edit Insight' : 'Write a New Insight'}</h1></div><div className="editor-header-actions"><span className={`status-pill status-${form.status}`}>{form.status === 'publish' ? 'Published' : form.status === 'future' ? 'Scheduled' : 'Draft'}</span><button className="btn btn-outline" onClick={() => onSave('draft')} disabled={saving || uploading}>{saving ? 'Saving…' : 'Save Draft'}</button>{form.status === 'future' ? <button className="btn btn-primary" onClick={() => onSave('schedule')} disabled={saving || uploading}>Schedule Insight</button> : <button className="btn btn-primary" onClick={() => onSave('publish')} disabled={saving || uploading}>{editing.id && form.status === 'publish' ? 'Update Insight' : 'Publish Insight'}</button>}</div></div>
    <div className="insight-editor-grid"><main className="insight-editor-main"><section className="editor-card">
      <div className="field"><label>Title</label><input className="title-input" value={form.title} onChange={e => setTitle(e.target.value)} placeholder="Write a clear, useful title…" autoFocus /></div>
      <div className="field"><div className="label-row"><label>Excerpt</label><span>{form.excerpt.length}/160</span></div><textarea value={form.excerpt} maxLength={160} rows={4} onChange={e => setField('excerpt', e.target.value)} placeholder="A short summary shown on the Insights listing…" /></div>
      <div className="field"><label>Article</label><RichTextEditor value={form.content} onChange={html => setField('content', html)} push={push}/><div className="editor-word-count">{countWords(stripHtml(form.content))} words</div></div>
      <div className="editor-section"><div className="editor-section-title">Publishing details</div><div className="editor-field-grid">
        <div className="field"><label>Author</label><select value={form.author} onChange={e => setField('author', e.target.value)}><option value="">Current user</option>{authors.map(author => <option key={author.id} value={author.id}>{author.name || author.slug}</option>)}</select></div>
        <div className="field"><label>Publish date</label><input type="datetime-local" value={form.date} onChange={e => setField('date', e.target.value)} /></div>
        <div className="field"><label>Status</label><select value={form.status} onChange={e => setField('status', e.target.value)}><option value="draft">Draft</option><option value="future">Scheduled</option><option value="publish">Published</option></select></div>
      </div>
      <div className="field"><label>URL slug</label><input value={form.slug} onChange={e => setSlug(e.target.value)} placeholder="your-insight-slug"/><span className="field-hint">{editing.slugManual ? 'Manual slug. Changing it may change the live URL.' : 'Generated from the title until you edit it.'}</span></div>
      <div className="editor-field-grid"><TaxonomyBox label="Category" items={categories} selected={form.categories} onChange={ids => setField('categories', ids)} newValue={newCategory} setNewValue={setNewCategory} onAdd={addCategory}/><TaxonomyBox label="Tags" items={tags} selected={form.tags} onChange={ids => setField('tags', ids)} newValue={newTag} setNewValue={setNewTag} onAdd={addTag}/></div></div>
    ...
    <aside className="insight-editor-side"><section className="editor-card seo-card"><div className="card-kicker">SEARCH OPTIMIZATION</div><h2>SEO</h2><p className="side-intro">Improve how this Insight can appear in search without blocking publication.</p>
      <div className="field"><div className="label-row"><label>Focus keyword</label><span>{form.focus_keyword.length}/80</span></div><input value={form.focus_keyword} maxLength={80} onChange={e => setField('focus_keyword', e.target.value)} placeholder="e.g. freight forwarding in Nigeria"/></div>
      <div className="field"><div className="label-row"><label>SEO title</label><span>{form.seo_title.length}/60</span></div><input value={form.seo_title} maxLength={60} onChange={e => setField('seo_title', e.target.value)} placeholder={form.title || 'Falls back to Insight title'}/></div>
      <div className="field"><div className="label-row"><label>Meta description</label><span>{form.meta_description.length}/160</span></div><textarea value={form.meta_description} maxLength={160} rows={5} onChange={e => setField('meta_description', e.target.value)} placeholder={form.excerpt || 'Falls back to excerpt'}/></div><SeoScoreCard seo={seo}/>
    </section>
    <section className="editor-card google-card"><div className="card-kicker">SEARCH PREVIEW</div><h2>Google preview</h2><div className="google-preview"><div className="google-url">{previewUrl}</div><div className="google-title">{googleTitle}</div><div className="google-description">{googleDescription}</div></div></section>
    <section className="editor-card publishing-card"><div className="card-kicker">PUBLISHING</div><h2>Ready to publish?</h2><p>{form.status === 'publish' ? 'Changes will update the live Insight.' : form.status === 'future' ? 'This Insight is set to publish at the selected date.' : 'Save as a draft while the article is being prepared.'}</p><div className="publishing-actions"><button className="btn btn-outline" onClick={() => onSave('draft')} disabled={saving || uploading}>Save draft</button><button className="btn btn-primary" onClick={() => onSave(form.status === 'future' ? 'schedule' : 'publish')} disabled={saving || uploading}>{form.status === 'future' ? 'Schedule' : form.status === 'publish' ? 'Update live Insight' : 'Publish now'}</button></div></section></aside></div>
  </div>;
}

function TaxonomyBox({ label, items, selected, onChange, newValue, setNewValue, onAdd }) {
  function toggle(id) { const next = new Set((selected || []).map(Number)); const numeric = Number(id); next.has(numeric) ? next.delete(numeric) : next.add(numeric); onChange([...next]); }
  return <div className="field"><label>{label}</label><div className="taxonomy-box">{items.length ? items.map(item => <label key={item.id}><input type="checkbox" checked={(selected || []).map(Number).includes(Number(item.id))} onChange={() => toggle(item.id)}/><span>{item.name}</span></label>) : <span className="field-hint">No {label.toLowerCase()}s yet.</span>}</div><div className="taxonomy-add"><input value={newValue} onChange={e => setNewValue(e.target.value)} onKeyDown={e => e.key === 'Enter' && (e.preventDefault(), onAdd())} placeholder={`New ${label.toLowerCase()}`}/><button type="button" className="btn btn-outline btn-sm" onClick={onAdd}>Add</button></div></div>;
}
...
function calculateSeo({ title, excerpt, content, slug, focus_keyword, seo_title, meta_description }) {
  const keyword = String(focus_keyword || '').trim().toLowerCase();
  const cleanTitle = String(title || '').trim();
  const cleanSeoTitle = String(seo_title || '').trim();
  const cleanDescription = String(meta_description || '').trim();
  const effectiveTitle = cleanSeoTitle || cleanTitle;
  const effectiveDescription = cleanDescription || String(excerpt || '').trim();
  const normalizedSlug = String(slug || '').toLowerCase();
  const body = String(content || '').toLowerCase();
  const wordCount = countWords(content);
  if (!keyword) return { score: 0, checks: [{ ok: false, label: 'Enter a focus keyword to begin' }, { ok: Boolean(cleanTitle), label: 'Insight title' }, { ok: wordCount >= 300, label: 'Content length' }] };
  let score = 0; const checks = [];
  const titleOk = cleanTitle.toLowerCase().includes(keyword); if (titleOk) score += 20; checks.push({ ok: titleOk, label: 'Keyword appears in the title' });
  const seoTitleOk = effectiveTitle.length >= 30 && effectiveTitle.length <= 60; if (seoTitleOk) score += 15; checks.push({ ok: seoTitleOk, label: 'SEO title is 30–60 characters' });
  const descriptionOk = effectiveDescription.length >= 120 && effectiveDescription.length <= 160; if (descriptionOk) score += 15; checks.push({ ok: descriptionOk, label: 'Meta description is 120–160 characters' });
  const slugOk = normalizedSlug.includes(keyword.replace(/\s+/g, '-')); if (slugOk) score += 10; checks.push({ ok: slugOk, label: 'Keyword appears in the URL slug' });
  const introOk = body.slice(0, 500).includes(keyword); if (introOk) score += 15; checks.push({ ok: introOk, label: 'Keyword appears near the beginning' });
  const lengthOk = wordCount >= 300; if (lengthOk) score += 15; checks.push({ ok: lengthOk, label: 'Article has at least 300 words' });
  const excerptOk = String(excerpt || '').trim().length >= 80; if (excerptOk) score += 10; checks.push({ ok: excerptOk, label: 'Excerpt is useful and descriptive' });
  return { score, checks };
}
